...
namespace Bovis.Service.Queries
{
    public class CostoQueryService : ICostoQueryService
    {
        ...
        #region GetCostos
        public async Task<Response<List<Costo_Detalle>>> GetCostos(bool? hist, string? idEmpleado, int? idPuesto, int? idProyecto, int? idEmpresa, int? idUnidadNegocio)
        {
            var response = await _costoBusiness.GetCostos(hist, idEmpleado, idPuesto, idProyecto, idEmpresa, idUnidadNegocio);
