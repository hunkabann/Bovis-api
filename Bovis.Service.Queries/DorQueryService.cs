...
namespace Bovis.Service.Queries
{
    public class DorQueryService : IDorQueryService
    {
        ...
        public async Task<Response<List<Dor_ObjetivosGenerales>>> GetDorObjetivosGenerales(int nivel, string unidadNegocio, int mes, int anio, string seccion)
        {
            var response = await _dorBusiness.GetDorObjetivosGenerales(nivel, unidadNegocio, mes, anio, seccion);
        ...
        public async Task<Response<List<Dor_ObjetivosGenerales>>> GetDorMetasProyecto(int proyecto, int nivel, int mes, int anio, int empleado, string seccion)
        {
            var response = await _dorBusiness.GetDorMetasProyecto(proyecto, nivel, mes, anio, empleado, seccion);
        ...
        public async Task<Response<List<Dor_ObjetivosEmpleado>>> GetDorObjetivoDesepeno(int anio, int proyecto, int empleado, int nivel, int? acepto, int mes)
        {
            var response = await _dorBusiness.GetDorObjetivosDesepeno(anio, proyecto, empleado, nivel, acepto, mes);
            return new Response<List<Dor_ObjetivosEmpleado>> { Data = _map.Map<List<Dor_ObjetivosEmpleado>>(response), Success = response is not null ? true : default, Message = response is null ? "No se encontró información para el usuario solicitado." : default };
        }
