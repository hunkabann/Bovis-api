...
public class AgregaEdoEventHandler : IRequestHandler<AgregarEdoCommand, Response<bool>>
{
    ...
    public AgregaEdoEventHandler(ICatalogoBusiness _business, IMapper _mapper)
    {
        this._business = _business;
        this._mapper = _mapper;
    }

    public async Task<Response<bool>> Handle(AgregarEdoCommand request, CancellationToken cancellationToken)
    {
        var resp = new Response<bool>();
        (bool Success, string Message) tmpResp = await _business.AddEdo(new TB_Estado { Activo = true, Estado = request.descripcion });
        if (!tmpResp.Success) resp.AddError(tmpResp.Message);
        else resp.Data = tmpResp.Success;
        return resp;
    }
...
public class EliminaEdoEventHandler : IRequestHandler<EliminarEdoCommand, Response<bool>>
{
    private readonly ICatalogoBusiness _business;

    public EliminaEdoEventHandler(ICatalogoBusiness _business)
    {
        this._business = _business;
    }

    public async Task<Response<bool>> Handle(EliminarEdoCommand request, CancellationToken cancellationToken)
    {
        var resp = new Response<bool>();
        (bool Success, string Message) tmpResp = await _business.DeleteEdo(new TB_Estado { IdEstado = request.id });
        if (!tmpResp.Success) resp.AddError(tmpResp.Message);
        else resp.Data = tmpResp.Success;
        return resp;
    }
...
public class ActualizaEdoEventHandler : IRequestHandler<ActualizarEdoCommand, Response<bool>>
{
    private readonly ICatalogoBusiness _business;

    public ActualizaEdoEventHandler(ICatalogoBusiness _business)
    {
        this._business = _business;
    }

    public async Task<Response<bool>> Handle(ActualizarEdoCommand request, CancellationToken cancellationToken)
    {
        var resp = new Response<bool>();
        (bool Success, string Message) tmpResp = await _business.UpdateEdo(new InsertMovApi { Rel = request.Rel, Nombre = request.Nombre, Roles = request.Roles, TransactionId = request.TransactionId, Usuario = request.Usuario }, new TB_Estado { Estado = request.descripcion, IdEstado = request.id, Activo = true });
        if (!tmpResp.Success) resp.AddError(tmpResp.Message);
        else resp.Data = tmpResp.Success;
        return resp;
    }
...
public class AgregaPaisEventHandler : IRequestHandler<AgregarPaisCommand, Response<bool>>
{
    private readonly ICatalogoBusiness _business;
    private readonly IMapper _mapper;

    public AgregaPaisEventHandler(ICatalogoBusiness _business, IMapper _mapper)
    {
        this._business = _business;
        this._mapper = _mapper;
    }

    public async Task<Response<bool>> Handle(AgregarPaisCommand request, CancellationToken cancellationToken)
    {
        var resp = new Response<bool>();
        (bool Success, string Message) tmpResp = await _business.AddPais(new TB_Pais { Activo = true, Pais = request.descripcion });
        if (!tmpResp.Success) resp.AddError(tmpResp.Message);
        else resp.Data = tmpResp.Success;
        return resp;
    }
...
public class EliminaPaisEventHandler : IRequestHandler<EliminarPaisCommand, Response<bool>>
{
    private readonly ICatalogoBusiness _business;

    public EliminaPaisEventHandler(ICatalogoBusiness _business)
    {
        this._business = _business;
    }

    public async Task<Response<bool>> Handle(EliminarPaisCommand request, CancellationToken cancellationToken)
    {
        var resp = new Response<bool>();
        (bool Success, string Message) tmpResp = await _business.DeletePais(new TB_Pais { IdPais = request.id });
        if (!tmpResp.Success) resp.AddError(tmpResp.Message);
        else resp.Data = tmpResp.Success;
        return resp;
    }
...
public class ActualizaPaisEventHandler : IRequestHandler<ActualizarPaisCommand, Response<bool>>
{
    private readonly ICatalogoBusiness _business;

    public ActualizaPaisEventHandler(ICatalogoBusiness _business)
    {
        this._business = _business;
    }

    public async Task<Response<bool>> Handle(ActualizarPaisCommand request, CancellationToken cancellationToken)
    {
        var resp = new Response<bool>();
        (bool Success, string Message) tmpResp = await _business.UpdatePais(new InsertMovApi { Rel = request.Rel, Nombre = request.Nombre, Roles = request.Roles, TransactionId = request.TransactionId, Usuario = request.Usuario }, new TB_Pais { Pais = request.descripcion, IdPais = request.id, Activo = true });
        if (!tmpResp.Success) resp.AddError(tmpResp.Message);
        else resp.Data = tmpResp.Success;
        return resp;
    }
...
public class EliminaPuestoEventHandler : IRequestHandler<EliminarPuestoCommand, Response<bool>>
{
	...
	{
		var resp = new Response<bool>();
		(bool Success, string Message) tmpResp = await _business.DeletePuesto(new TB_Cat_Puesto { IdPuesto = Convert.ToInt32(request.id.ToString()) });
		if (!tmpResp.Success) resp.AddError(tmpResp.Message);
		else resp.Data = tmpResp.Success;
		return resp;
	}
...
public class ActualizaPuestoEventHandler : IRequestHandler<ActualizarPuestoCommand, Response<bool>>
{
	...
	{
		var resp = new Response<bool>();
		(bool Success, string Message) tmpResp = await _business.UpdatePuesto(new InsertMovApi { Rel = request.Rel, Nombre = request.Nombre, Roles = request.Roles, TransactionId = request.TransactionId, Usuario = request.Usuario }, new TB_Cat_Puesto { Puesto = request.descripcion, IdPuesto = Convert.ToInt32(request.id.ToString()), Activo = true });
		if (!tmpResp.Success) resp.AddError(tmpResp.Message);
		else resp.Data = tmpResp.Success;
		return resp;
	}
